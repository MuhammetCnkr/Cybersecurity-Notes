...
> STDIN, STDOUT, STDERR, yönlendirme ve pipe veri akışını anlamak.
...
- `>` hedef dosyayı önce keser; `>>` sona ekler.
- Here-document (`<<EOF`) EOF’yi dosyaya eklemez; yalnızca girdinin nerede bittiğini belirleyen ayraçtır.
- `cat < file` geçerlidir ancak `cat file` ile aynı sonucu daha doğrudan verir.
- Globbing içeren `find -name "*.conf"` ifadesinde desen tırnaklanmalıdır.

## Eksik kalabilecek kavramlar

- `2>&1`, `&>`, process substitution
- pipe exit status ve `pipefail`
- named pipe

## Bilişsel bağlantılar

[[12-Filter Contents|Filtre zincirleri]] · [[18-Task Scheduling|Zamanlanmış iş logları]]

## Aktif tekrar / mini lab

```bash
`printf "ok\n" >out; ls /yok 2>err; { cat out; cat err; } | wc -l` ile akışı izle.
```
