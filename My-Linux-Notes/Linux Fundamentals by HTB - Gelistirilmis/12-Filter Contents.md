...
> Metin akışlarını küçük araçlarla seçmek, dönüştürmek ve saymak.
...
- `/etc/passwd` grup üyeliklerini değil yalnızca birincil GID alanını içerir; ek grup üyelikleri `/etc/group` içindedir.
- Base64 şifreleme değildir; kodlamadır ve gizlilik sağlamaz.
- Doğru komut `cat` olmalı; nottaki `vat` yazım hatasıdır.
- `cut -d":" -f1` ifadesinde `-d` ve `-f` seçenekleri ayrıdır.
- `ss` bir içerik filtresi değil ağ soketi inceleme aracıdır; [[6-System Information]] veya [[19-Network Services]] ile ilişkilidir.

## Eksik kalabilecek kavramlar

- `grep -F` vs `grep -E`
- locale’un `sort` etkisi
- CSV gibi karmaşık biçimlerde basit delimiter sınırları

## Bilişsel bağlantılar

[[13-Regular Expressions|Regex]] · [[11-File Descriptors and Redirections|Pipe]]

## Aktif tekrar / mini lab

```bash
`getent passwd | awk -F: "$3>=1000 {print $1, $7}" | sort | uniq -c` zincirini parça parça açıkla.
```
