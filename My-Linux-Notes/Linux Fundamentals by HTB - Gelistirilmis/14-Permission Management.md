...
> Linux sahiplik ve rwx izinlerini dosya/dizin bağlamında yorumlamak.
...
- Dizinlerde `r` adları listeleme, `w` giriş oluşturma/silme, `x` ise dizini traverse etme anlamına gelir.
- Bir dosyayı silme yetkisi çoğunlukla dosyanın kendi izninden değil üst dizinin izinlerinden gelir.
- Sayısal modlarda `r=4`, `w=2`, `x=1`; üçlüler owner/group/other sırasındadır.
...
[[15-User Management|Kullanıcı/grup]] · [[9-Editing Files|Hassas dosyalar]]

## Aktif tekrar / mini lab

```bash
`mkdir /tmp/perm-lab; touch /tmp/perm-lab/a; chmod 640 /tmp/perm-lab/a; stat -c "%A %a %U:%G" /tmp/perm-lab/a`.
```
