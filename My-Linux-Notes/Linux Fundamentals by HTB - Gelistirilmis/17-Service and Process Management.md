...
> Süreç, job, signal ve systemd servis durumlarını ayırmak.
...
- Process, çalışan program örneğidir; service genellikle arka planda uzun süre çalışan ve yönetilen süreçtir.
- `kill` varsayılan olarak SIGTERM gönderir; SIGKILL son çare olmalıdır çünkü süreç temizlik yapamaz.
- `systemctl enable` servisi hemen başlatmak zorunda değildir; `--now` ikisini birleştirir.

## Eksik kalabilecek kavramlar

- PID/PPID
- foreground/background ve jobs
- signals
- journalctl

## Bilişsel bağlantılar

[[18-Task Scheduling|Zamanlama]] · [[19-Network Services|Ağ daemonları]]

## Aktif tekrar / mini lab

```bash
`ps -eo pid,ppid,stat,cmd --forest | head -30; systemctl --type=service --state=running | head`.
```
