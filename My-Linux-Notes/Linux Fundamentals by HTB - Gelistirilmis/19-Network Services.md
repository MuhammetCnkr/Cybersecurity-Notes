...
> SSH, NFS, web ve VPN servislerinin amaç, port ve güvenlik sınırlarını kavramak.
...
- Ubuntu NFS sunucu paketi `nfs-kernel-server`, istemci araçları `nfs-common` paketindedir; “NFS-UTILS (Ubuntu)” ifadesi dağıtım ayrımını karıştırır.
- NFS, FTP’nin genel yerine geçen bir protokol değildir; dosya sistemi paylaşımı ve güven modeli farklıdır.
- OpenSSH ayar dosyası genellikle `/etc/ssh/sshd_config`; değişiklikten önce `sshd -t` ile sözdizimi doğrulanmalıdır.
- Web sunucusu doğrudan “dinamik HTML oluşturmak” zorunda değildir; statik içerik sunabilir veya uygulama sunucusuna reverse proxy olabilir.

## Eksik kalabilecek kavramlar

- portlar 22/80/443/2049
- bind address ve firewall
- TLS
- least exposure

## Bilişsel bağlantılar

[[20-Working with Web Services|Apache/HTTP]] · [[6-System Information|ss/ip]]

## Aktif tekrar / mini lab

```bash
`ss -lntup` çıktısında dinleyen servis, adres ve portu eşleştir; yalnızca yetkili lab ortamında test et.
```
